...
// ---------------------------------------------
// OPERATIONAL CONTEXT DETERMINATION
// ---------------------------------------------
$isWalletFunding = isset($_GET['fund-wallet']) && (int)$_GET['fund-wallet'] === 1;

// ---------------------------------------------
// GET ORDER OR AMOUNT DEPENDING ON CONTEXT
// ---------------------------------------------
$order_ids = [];
$total_amount = 0.00;

// Connect Database early to fulfill routing prerequisites
try {
...
if ($isWalletFunding) {
    ...
    $total_amount = isset($_REQUEST['amount']) ? max(0, (float)$_REQUEST['amount']) : 0;
    if ($total_amount <= 0) {
        $_SESSION['flash_error'] = "Please specify a valid funding amount allocation.";
        header("Location: fund-wallet.php");
        exit;
    }
} else {
    // Context: Traditional Ticket Checkout
...
if (!empty($gatewayDetails['instruction'])) {
    $instructions = $gatewayDetails['instruction'];
}
...
if ($isBalancePayment && !$isWalletFunding) {
    if ($user_balance < $final_payable_amount) {
        ...
        header("Location: fund-wallet.php?amount=" . urlencode($final_payable_amount - $user_balance));
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_balance_payment'])) {
        try {
            ...
            $logDeposit = $pdo->prepare("
                INSERT INTO deposits (user_id, payment_id, order_ids, amount, currency, payment_type, submitted_details, proof_file, status, description) 
            ...
            $logDeposit->execute([
                ...
                "Settled Ticket Order(s) #{$order_ids_string} using Instant Wallet Balance"
            ]);
...
// ---------------------------------------------
// POST FORM HANDLING: TRADITIONAL / FUND-WALLET MANUAL UPLOADS
// ---------------------------------------------
$errors = [];
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$isBalancePayment) {
    $submittedData = [];
    
    // Evaluate variant rules
    ...
    if (empty($errors)) {
        ...
        $order_ids_string = !$isWalletFunding ? implode(',', $order_ids) : null;
        
        // Formulate target description text
        if ($isWalletFunding) {
            $logDescription = "Wallet Funding Request via " . strtoupper($payment_type);
            $successDashboardMsg = "Your wallet funding deposit request was uploaded successfully! Awaiting validation.";
        } else {
            $logDescription = "Manual Ticket Checkout Gateway Payment for Order(s) #{$order_ids_string}";
            $successDashboardMsg = "Your ticket order payment was sent successfully! Check back shortly.";
        }
        
        try {
            $pdo->beginTransaction();

            // 1. Insert Deposit Entry
            ...
            // 2. Conditional Orders Processing Row Changes
            if (!$isWalletFunding) {
                $updatePlaceholders = implode(',', array_fill(0, count($order_ids), '?'));
                $updateStmt = $pdo->prepare("
                    UPDATE orders 
            ...
            $_SESSION['flash_success'] = $successDashboardMsg;
            header("Location: dashboard");
            exit;
            ...
            $errors[] = "Failed saving parameters to local database logs: " . $e->getMessage();
        }
    }
}
?>
        ...
        <div class="bg-gradient-to-r from-blue-800 to-indigo-900 p-8 text-white flex items-center justify-between">
            <div>
                ...
                <h1 class="text-3xl font-black mt-2">
                    <?= $isWalletFunding ? "Account Wallet Funding" : ($isBalancePayment ? "Wallet Debit Confirmation" : "Gateway Validation") ?>
                </h1>
            </div>
        ...
        <div class="p-8">
            ...
            <?php if ($isBalancePayment && !$isWalletFunding): ?>
                <div class="bg-emerald-50 border border-emerald-200 rounded-2xl p-6 mb-8 text-center">
                    ...
                    <p class="text-slate-600 mb-4">You are checking out via your internal digital voucher pool account balance.</p>
                ...
                <div class="bg-slate-50 border border-slate-200 rounded-2xl p-6 mb-8">
                    ...
                    <?php if ($payment_type === 'gift_card'): ?>
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div class="shrink-0">
                                <a href="../giftcard" target="_blank" class="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-wider px-4 py-2.5 rounded-xl shadow-sm transition-all">
                                    <i class="fas fa-shopping-bag text-[11px]"></i> Buy Giftcard Guide
                                </a>
                            </div>
                        </div>
                ...
                <form method="POST" action="" enctype="multipart/form-data" class="space-y-6">
                    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
                        ...
                        <div class="p-4 bg-emerald-50 text-emerald-900 rounded-xl flex justify-between items-center mb-4">
                            <span class="text-sm font-medium"><?= $isWalletFunding ? "Target Top Up Deposit Value:" : "Required Payable Amount:" ?></span>
                        ...
                        <?php if ($payment_type === 'gift_card'): ?>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="card_code" required class="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-blue-500 font-mono" placeholder="XXXX-XXXX-XXXX-XXXX">
                                </div>
                                <div>
                                    <label class="block text-xs uppercase font-bold text-slate-500 mb-1">Card Security Code / Pin (Optional)</label>
                                    <input type="text" name="card_pin" class="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-blue-500 font-mono" placeholder="1234">
                                </div>
                            </div>
                        <?php elseif ($payment_type === 'bank'): ?>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="sender_bank" required class="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-blue-500" placeholder="e.g. Chase Bank">
                                </div>
                                <div>
                                    <label class="block text-xs uppercase font-bold text-slate-500 mb-1">Sender Bank Account Holder Name</label>
                                    <input type="text" name="sender_account_name" required class="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-blue-500" placeholder="John Doe">
                                </div>
                            </div>
                        <?php elseif ($payment_type === 'crypto'): ?>
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="txid_hash" required class="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-blue-500 font-mono" placeholder="0xabc123...">
                                </div>
                                <div>
                                    <label class="block text-xs uppercase font-bold text-slate-500 mb-1">Your External Wallet Outgoing Address (Optional)</label>
                                    <input type="text" name="sender_wallet" class="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-blue-500 font-mono" placeholder="Source wallet hash">
                                </div>
                            </div>
                        <?php elseif ($payment_type === 'e_pay'): ?>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="sender_email_phone" required class="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-blue-500" placeholder="john.doe@example.com">
                                </div>
                                <div>
                                    <label class="block text-xs uppercase font-bold text-slate-500 mb-1">Transaction Ref / Order Reference</label>
                                    <input type="text" name="transaction_reference" required class="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:border-blue-500 font-mono" placeholder="PAY-998231">
                                </div>
                            </div>
                        <?php endif; ?>
                    ...
                    <div class="flex items-center justify-between gap-4 pt-4">
                        <a href="<?= $isWalletFunding ? 'fund-wallet.php' : 'checkout' ?>?currency=<?php echo urlencode($displayCurrency); ?>" class="bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold px-6 py-3.5 rounded-xl text-center transition text-sm">
                        ...
                        <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-black px-6 py-3.5 rounded-xl shadow-lg hover:shadow-xl transition text-center tracking-wide text-sm">
                            Confirm & Process Payment Data <i class="fas fa-shadow-alt ml-1"></i>
                        </button>
                    </div>
                </form>
            <?php endif; ?>
